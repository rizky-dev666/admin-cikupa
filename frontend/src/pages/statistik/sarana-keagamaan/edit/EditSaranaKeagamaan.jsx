...
const EditSaranaKeagamaan = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await axios.get(`/api/data-sarana-keagamaan/${id}`);
        setDataSaranaKeagamaan(res.data);
      } catch (error) {
  ...
  const handleSubmitSaranaKeagamaan = async (e) => {
    e.preventDefault();
    try {
      await axios.put(`/api/data-sarana-keagamaan/${id}`, dataSaranaKeagamaan);
      alert("Data sarana keagamaan berhasil diperbarui!");
      navigate("/statistik/sarana-keagamaan");
    } catch (error) {
      alert("Gagal update data sarana keagamaan");
      console.error(error);
    }
  };
...
export default EditSaranaKeagamaan;

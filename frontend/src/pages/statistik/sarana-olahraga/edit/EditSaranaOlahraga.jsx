...
const EditSaranaOlahraga = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await axios.get(`/api/data-sarana-olahraga/${id}`);
        setDataSaranaOlaharaga(res.data);
      } catch (error) {
  ...
  const handleSubmitSaranaOlahraga = async (e) => {
    e.preventDefault();
    try {
      await axios.put(`/api/data-sarana-olahraga/${id}`, dataSaranaOlahraga);
      alert("Data sarana olaharaga berhasil diperbarui!");
      navigate("/statistik/sarana-olahraga");
    } catch (error) {
      alert("Gagal update data sarana olahraga");
      console.error(error);
    }
  };
  ...
  return (
    <div style={{ padding: 20 }}>
      <h2>Edit Data sarana olahraga</h2>
...
export default EditSaranaOlahraga;
